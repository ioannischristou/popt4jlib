echo off
REM runsSTnbinSimpleHeurOptDBatchTest.bat <Kr> <Ko> <L> <lambda> <p_l> <h> <p> <serverport>(7891) <numworkerthreads>(8) [epst(0.01)] [tnot(0.01)] [bsize(24)] [dbglvl(0)] [run4ZeroOrderCost(true)]
...
pause
echo on
REM java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.nbin.sSTCnbinSimpleHeurOpt <Kr> <Ko> <L> <lambda> <p_l> <h> <p> [pdsrvhost(localhost)] [pdsrvport(7891)] [epst(0.01)] [tnot(0.01)] [bsize(24)] [dbglvl(0)] [run4ZeroOrderCost(true)]
java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.nbin.sSTCnbinSimpleHeurOpt %Kr% %Ko% %L% %lambda% %pl% %h% %p% localhost %srvport% %prest%
