echo off
REM runRnQTpoissonHeurPOptDBatchTest.bat <Kr> <Ko> <L> <lambda> <h> <p> <p2(0)> <serverport>(7891) <numworkerthreads>(8) [bsize(24)] [epst(0.01)] [tnot(0.01)] [dbglvl(0)]
...
pause
echo on
REM java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.rnqt.poisson.RnQTCpoissonHeurPOpt <Kr> <Ko> <L> <lambda> <h> <p> [p2(0)] [pdsrvhost(localhost)] [pdsrvport(7891)] [bsize(24)] [epst(0.01)] [tnot(0.01)] [dbglvl(0)]
@echo off
...
@echo on
java -Xmx4000m -cp .\dist\popt4jlib.jar tests.sic.rnqt.poisson.RnQTCpoissonHeurPOpt %p1% %p2% %p3% %p4% %p5% %p6% %p7% localhost %p8% %prest%
