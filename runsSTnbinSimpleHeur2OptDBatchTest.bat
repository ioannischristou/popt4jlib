echo off
REM runsSTnbinSimpleHeur2OptDBatchTest.bat <Kr> <Ko> <L> <lambda> <p_l> <h> <p> <serverport>(7891) <numworkerthreads>(24) [epst(0.01)] [tnot(0.01)] [bsize(24)] [dbglvl(0)] [run4zeroordercost(true)]
...
pause
echo on
REM java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.nbin.sSTCnbinSimpleHeur2Opt <Kr> <Ko> <L> <lambda> <p_l> <h> <p> [pdsrvhost(localhost)] [pdsrvport(7891)] [epst(0.01)] [tnot(0.01)] [bsize(24)] [dbglvl(0)] [run4zeroordercost(true)]
java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.nbin.sSTCnbinSimpleHeur2Opt %Kr% %Ko% %L% %lambda% %p_l% %h% %p% localhost %srvport% %prest%
