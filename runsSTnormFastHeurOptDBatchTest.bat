echo off
REM runsSTnormFastHeurOptDBatchTest.bat <Kr> <Ko> <L> <mu> <sigma> <h> <p> <p2>(0) <serverport>(7891) <numworkerthreads>(8) [epst(0.01)] [deltat(0.1)] [epss(1.e-4)] [qnot(1.e-6)] [bsize(24)] [dbglvl(0)]
...
pause
echo on
REM java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.norm.sSTCnormFastHeurOpt <Kr> <Ko> <L> <mi> <sigma> <h> <p> [p2(0)] [pdsrvhost(localhost)] [pdsrvport(7891)] [epst(0.01)] [deltat(0.1)] [epss(1.e-4)] [qnot(1.e-6)] [bsize(24)] [dbglvl(0)]
java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.norm.sSTCnormFastHeurOpt %Kr% %Ko% %L% %mi% %sigma% %h% %p% %p2% localhost %srvport% %prest%
