echo off
REM runsSTnbinFastHeurOptDBatchTest.bat <Kr> <Ko> <L> <lambda> <p_l> <h> <p> <serverport>(7891) <numworkerthreads>(24) [epst(0.01)] [deltat(0.1)] [bsize(24)] [dbglvl(0)] [zeroOrderCost(true)]
...
pause
echo on
REM java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.nbin.sSTCnbinFastHeurOpt <Kr> <Ko> <L> <lambda> <p_l> <h> <p> [pdsrvhost(localhost)] [pdsrvport(7891)] [epst(0.01)] [deltat(0.1)] [bsize(24)] [dbglvl(0)] [zeroOrderCost(true)]
java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.nbin.sSTCnbinFastHeurOpt %Kr% %Ko% %L% %lambda% %pl% %h% %p% localhost %srvport% %prest%
