...
pause
echo on
REM java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.poisson.sSTCpoissonFastHeurOpt <Kr> <Ko> <L> <lambda> <h> <p> [p2(0)] [pdsrvhost(localhost)] [pdsrvport(7891)] [epst(0.01)] [deltat(0.1)] [bsize(24)]
java -Xmx8000m -cp .\dist\popt4jlib.jar tests.sic.sST.poisson.sSTCpoissonFastHeurOpt %Kr% %Ko% %L% %lambda% %h% %p% %p2% localhost %srvport% %prest%
