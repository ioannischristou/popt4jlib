 ...
 * @version 1.0
 */
public class HPartitionerMLSPP2 implements HPartitionerIntf {
  ...
  /**
   * sets the objective function to be minimized by the partitioner.
   * @param f HObjFncIntf
   */
  ...
  /**
   * the main class method. A number of properties must be passed in the 3rd
   * argument.
   * @param g HGraph
   * @param k int
  ...
  public int[] partition(HGraph g, int k, Hashtable props) throws PartitioningException {
    ...
    HPartitionerIntf parter = (HPartitionerIntf) props.get("partitioning.HPartitioner.HPartitionerMLSPP.basepartitioner");
    int num_tries = 1;
    ...
    try {
      SPP2MPSWriterUtil.writeSPP2MPSFileFast(sppmaker.getConstraintsMatrix(), sppmaker.getCostVector(), k, filename);
    ...
    HPartitionerIntf refiner = (HPartitionerIntf) props.get("partitioning.HPartitioner.HPartitionerMLSPP.refiner");
