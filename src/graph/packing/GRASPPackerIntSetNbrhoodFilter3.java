...
class GRASPPackerIntSetNbrhoodFilter3 implements IntSetNeighborhoodFilterIntf {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        Node rni = _g.getNodeUnsynchronized(rmid.intValue());
			...
			List res = new ArrayList(nbors.size());
  ...
  /**
   * provide a List&ltIntSet&gt of sets of two integers to be removed together 
	 ...
	 * neighbors of x at distance 4. Essentially, returns a list of sets of pairs 
	 * of nodes &ltx,y&gt for each y in arg, that happens to be at distance &lte 4 
	 * from x.
   * @param x // Integer
  ...
  public List filter(Integer x, Set arg, Hashtable params) throws LocalSearchException {
    try {
      ...
      // Set nxnnborids = nx.getNNborIndicesUnsynchronized();
			Set nxnnbors = nx.getNNborsUnsynchronized();
      ...
      while (it.hasNext()) {
        ...
        // boolean ok = nynborids.size()>0;
				Set nynnbors = ny.getNNborsUnsynchronized();
