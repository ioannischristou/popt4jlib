...
public class AllMWCFinder {
  ...
  public AllMWCFinder(Graph g) throws GraphException {
  ...
  public Set getAllMaximalCliques(double minaccnodecliqueweight) {
    try {
      ...
      for (int i = 0; i < n; i++) {
        nodeweights[i] = _g.getNode(i).getArcWeights();
      }
      for (int i = 0; i < n; i++) {
        ...
        Set oldcliquesi = new TreeSet(); // Set<Set<Integer id> >
        while (expandCliquesBy1(_g, maxcliquesi, newcliquesi, oldcliquesi,
                                minaccnodecliqueweight)) {
          addUnique(oldcliquesi, max_cliques);
  ...
  private static boolean expandCliquesBy1(Graph g, Set cliques, Set newcliques, Set oldcliques, double minval) {
    Iterator iter = cliques.iterator();
    while (iter.hasNext()) {
      Set clique = (Set) iter.next();
      Vector fullnbors = g.getFullNbors(clique, minval);
      if (fullnbors.size()==0) oldcliques.add(clique);
      else {
