...
public class DActiveMsgPassingLongLivedConnTest {
   ...
   * <CODE>java -cp &lt;classpath&gt; 
	 * parallel.distributed.DMsgPassingLongLivedConnTest 
	 * &lt;startid&gt; &lt;ndid&gt; [delay(-1)]</CODE>.
   * The test is set up so that the
   * <CODE>parallel.distributed.DActiveMsgPassingCoordinatorLongLivedConnSrv</CODE> 
	 * must be up and running at the default (host=localhost,port=7895) 
	 * combination, and there must be exactly two processes, one started with 
	 * (0,9) args and the other with (10,19) aguments.
   * @param args String[]
   */
  public static void main(String[] args) {
    try {
      ...
      DActiveMsgPassingCoordinatorLongLivedConnClt coordclt = 
				new DActiveMsgPassingCoordinatorLongLivedConnClt();  // srv is localhost
      ...
      if (startid==0) {
        coordclt.sendData(-1, 0, new DActiveMsgPassingLLCTestSerObject2()); 
        // bootstrap the recv->send cycle
	...
	static class DMPThread2 extends Thread {
		...
		public DMPThread2(int i) throws IOException {
			_id = i;
			_coordclt = new DActiveMsgPassingCoordinatorLongLivedConnClt();  
      // use localhost as server
		...
		public void run() {
			try {
				int sendTo = _id + 1;
				if (sendTo >= 20) sendTo = 0; // two JVMs running DMsgPassingTest
				// the first will start with args 0 9 and the second will start with
				// args 10 19
				for (int i = 0; i < 100; i++) {
					...
					while (!done) {
						try {
							_coordclt.sendData(_id, sendTo, 
								                 new DActiveMsgPassingLLCTestSerObject2());
							done=true;
						}
						catch (Exception e) {
							e.printStackTrace();
