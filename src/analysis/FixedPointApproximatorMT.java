...
public class FixedPointApproximatorMT {
  ...
  public VectorIntf findFixedPoint(VecFunctionIntf f, HashMap params, VectorIntf x0, long max_iters, double tol) throws ParallelException {
    ...
    while (cont && ++count<=max_iters) {
      utils.Messenger.getInstance().msg("FixedPointApproximatorMT.findFixedPoint(): starting iteration #"+count, 1);
      tasks.clear();
      int j=0;
      int k=len;
      for (int i = 0; i < _numThreads - 1; i++) {
        FixedPointTask fpti = new FixedPointTask(f, j, k-1, x, params, xstar);
        j = k;
        k += k;
      ...
      utils.Messenger.getInstance().msg("FixedPointApproximatorMT.findFixedPoint(): ||f(x)-x||="+d,1);
      cont = Math.abs(d)>=tol;
      if (cont) {
        VectorIntf tmp = x;
        x = xstar;
