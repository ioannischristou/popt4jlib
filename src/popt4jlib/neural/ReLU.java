...
public class ReLU extends BaseNNNode implements NNNodeIntf {
	...
	public double evalB(double[] inputSignals, double[] weights, int offset) {
		...
		if (Double.compare(prod, 0.0) > 0) setLastDerivEvalCache2(1.0);
		else setLastDerivEvalCache2(0.0);
		double result = Math.max(prod, 0.0);
		// cache values for speeding up auto-differentiation
		setLastInputsCache(inputSignals);
		setLastEvalCache(result);
	 ...
	 * The derivative does not exist at zero for this node as the left and right 
	 * derivative differ; we set it to zero there as well.
	 * @param weights double[] all variables (including biases) array
	 * @param index int the index of the partial derivative to take
	 * @param inputSignals double[]
	 * @param true_lbl double
	...
	public double evalPartialDerivativeB(double[] weights, int index, 
		...
		//    it's zero.
			...
			if (last_inputs!=null) {
				...
				*/
				final int pos = getDirectInputWeightStartIndex();
				double d10 = getLastDerivEvalCache2();
				if (!Double.isNaN(d10)) {
					double result = last_inputs[index-pos]*d10;
					setGradVectorCache(index, result);
					return result;
				}
				double node_input=_linearUnit.evalB(last_inputs,weights,pos);
				d10 = Double.compare(node_input, 0) > 0 ? 1.0 : 0.0;
				double result = d10*last_inputs[index-pos];
				//double result = Double.compare(node_input, 0) > 0 ?
				//	                last_inputs[index-pos] : 0.0;
				//setLastDerivEvalCache(result);
				setLastDerivEvalCache2(d10);
				setGradVectorCache(index, result);
				return result;
			}
			// nope, not in cache, do the work
			...
			for (int i=0; i<num_hidden_layers; i++) {
				...
				for (int j=0; j<layeri.length; j++) {
					...
					if (node_i_j == this) {  // this is the node we're looking for
						...
						//double result = Double.compare(node_input, 0) > 0 ?
						//	                layer_i_inputs[index-pos] : 0.0;
						double d10 = Double.compare(node_input, 0) > 0 ? 1.0 : 0.0;
						double result = d10*layer_i_inputs[index-pos];
						setLastInputsCache(layer_i_inputs);
						//setLastDerivEvalCache(result);
						setLastDerivEvalCache2(d10);
						setGradVectorCache(index, result);
