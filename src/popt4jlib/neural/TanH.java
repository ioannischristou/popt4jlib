...
/**
 * TanH implements the tanh function as activation function of a node. Can only
 * be used as hidden layer node. The class knows how to differentiate itself 
...
public class TanH extends BaseNNNode implements NNNodeIntf {
	...
	public double evalPartialDerivativeB(double[] weights, int index, 
		...
		// 2. if index is for bias, derivative is tPrime(nodeinput)
			...
			if (last_inputs!=null) {
				...
				/*
				// elseif condition below is impossible for TanH as it doesn't implement
	...
	public double evalPartialDerivativeB(double[] weights, int index, 
		...
		//    applied to this weight multiplied by tPrime(node_input)
			...
			/*
			// if condition below is impossible, as TanH doesn't implement the
			// OutputNNNodeIntf
			if (output_node==this) {  // our node is the output node
  			double node_input = _linearUnit.evalB(layer_i_inputs, weights, pos);
				double tp = tPrime(node_input);
	...
	/**
	 * the derivative of the tanh function as a function of one variable.
	 * @param x double
	 * @return double
	 */
