...
public class BoolVector {
  ...
  public void clear() {
	...
	public void copy(BoolVector other) throws IllegalArgumentException {
		if (_data.length !=other._data.length) 
			throw new IllegalArgumentException("argument has different size than this");
		// ok do the copy
  ...
  public int nextSetBit(int i) {
	...
	public void and(BoolVector other) throws IllegalArgumentException {
		if (other==null) throw new IllegalArgumentException("null argument passed in");
	...
	public void andParallel2(BoolVector other) throws IllegalStateException {
		if (_threads==null) throw new IllegalStateException("pool has been shut-down");
	...
	*/
	...
	static class BVAux {
		...
		synchronized boolean getFinish() { return _finish; }
		synchronized void setFinish() { _finish=true; notify(); }
		...
		synchronized void runFromTo(int starti, int endi, long[] data, long[] other_data, int what2run) {
			while (_starti!=-1) {
				try {
					wait();
				}
			...
			// ok, set values
			_starti=starti; _endi=endi; _data=data; _otherData=other_data;
			_what2Run = what2run;
			notify();
		}
		private synchronized void go1() {
			...
			// reset values
			_starti=-1; _endi=-1; _data=null; _otherData=null; _what2Run = -1;
			notify();
		}
	}
