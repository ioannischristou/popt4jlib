...
public class FNPTask3 implements ThreadSpecificTaskObject {
  ...
  private long _myid;
	...
	private int _originalthreadid;  // the thread from whose pool it is "borrowed"
	private int _threadid = Integer.MAX_VALUE;  // indicate it can run anywhere

	private final static boolean _DO_RELEASE_SANITY_CHECK=false;  // compile-time constant
  private static DynamicAsynchTaskExecutor3 _executor=null;  // set once from main()
  ...
  private static long _id = 0;  // object id
	private static final boolean _DO_COLLECT_STATS = false;  // compile-time constant
  ...
  private static ThreadLocal _localIncs = new ThreadLocal() {  // for optimization purposes only
	...
	// pool related data
	private FNPTask3Pool _pool=null;
	private boolean _isUsed = false;

	
  public FNPTask3(long id, long[] desc_numbers, long array_sum, long subtractions, long additions) {
    _myid = id;
  ...
  public void run() {
    ...
    if (!_do_runAux2 && _threadid!=Integer.MAX_VALUE) {  // only task is to release me
    ...
    incrNumNodesDone();
		...
		if (isManaged()) {  // next code is for releasing this task back to the pool
			...
			if (_originalthreadid==cur_thread_id) {  // short-cut?
				...
				if (_do_runAux2) throw new Error("shouldn't happen");  // valid only with new runAux2(), not runAux()
  ...
  private Serializable runAux2(int depth) {
	    ...
	    FNPTask3 left_node = null;
			...
			if (exec_cond_basic && !exec_cond_numthreads) {
				...
				left_node._sz = _sz-1;
			} else left_node = newInstance(_descNumbers.length, _sz-1);  // try the pool
			long[] left_branch = left_node._descNumbers;
			boolean keep_checking = true;
			for (int i=0; i<_sz-1; i++) {
				if (i==left_node._sz-1) {  // i==left_branch.length-1
	    ...
	    if (exec_cond_basic) { // ok, submit to executor
				...
				try {
					if (exec_cond_numthreads) {
						...
						_executor.submitToSameThread(left_node);  // mostly send nodes to hot-local queue
					}
					else _executor.execute(left_node);
				}
				catch (Exception e) {
				...
				try {
					if (exec_cond_numthreads) {
						...
						_executor.submitToSameThread(right_node);  // mostly send nodes to hot-local queue
					}
					else _executor.execute(right_node);
				}
				catch (Exception e) {
  ...
  public void release() {
    if (_pool!=null) {
			if (_DO_RELEASE_SANITY_CHECK) {
				...
				if (p!=_pool) {
					String str = "Current-Thread-id="+((IdentifiableIntf) Thread.currentThread()).getId();
					str += " _threadid="+_threadid+" _originalthreadid="+_originalthreadid;
  ...
  public static void main(String[] args) {
    ...
    Runtime.getRuntime().addShutdownHook(new Thread() {
      public void run() {
        System.err.println("best soln="+FNPTask3._incumbent+" Total nodes exec'd in executor="+FNPTask3._id+" _numNodesDone="+FNPTask3._numNodesDone);
				if (_DO_COLLECT_STATS) {
					System.err.println("Total FNPTask3 Objects Created="+FNPTask3._totalNumObjs+" Max Open Nodes="+FNPTask3._maxOpenNodes);
					System.err.println("Total Nodes Created (FNPTask3s + #setData() calls)="+FNPTask3._numTotalNodes);
					System.err.println("Total locks required for releasing FNPTask3 objects="+FNPTask3._totalNumLocks4Release);
				}
    ...
    System.err.println();
    try {
			_executor = DynamicAsynchTaskExecutor3.newDynamicAsynchTaskExecutor3(2, numthreads);
      ...
      while (!done()) {  // used to be !foundOptimalSolution() && !done()
        Thread.sleep(1000);
				synchronized(FNPTask3.class) {
					...
					if (_DO_COLLECT_STATS) 
						System.err.println("locks requested 4 releases so far=" + _totalNumLocks4Release);
				}
      }
			...
			if (_DO_COLLECT_STATS) {
				System.out.println("Total Nodes Created="+FNPTask3._numTotalNodes+
								           " Max Open Nodes="+FNPTask3._maxOpenNodes+
													 " locks requested 4 releases="+_totalNumLocks4Release);      
			}
  ...
  private synchronized static void incrNumNodesDone() {
    ...
    */
	...
	private void setData(long id, long array_sum, long subtractions, long additions) {
    _myid = id;
    _numbersSum = array_sum;
    _numAdds = additions;
    _numSubs = subtractions;
