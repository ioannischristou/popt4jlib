 ...
 * @version 1.0
 */
public class FletcherConjugateGradientST extends GLockingObserverBase 
   ...
   * <ul>
	 * <li>&lt;"fcg.x0", VectorIntf x&gt; optional, the initial starting point. If 
   ...
   * <li>&lt;"fcg.redrate", Double v&gt; optional, a user acceptable reduction 
	 ...
	 * the bracketing phase (used when fcg.fbar is not supplied.) Default is 2.0.
   ...
   * <li>&lt;"fcg.maxbracketingiters", Integer v&gt; optional, the maximum 
	 * allowed iterations in the bracketing phase of the Al-Baali - Fletcher 
	 * bracketing / sectioning algorithm for step-size determination. Default is 
	 * length(x)*100.
   * <li>&lt;"fcg.maxsectioningiters", Integer v&gt; optional, maximum allowed
  ...
  protected void notifyChangeProtected(SubjectIntf subject) 
    ...
    else throw new OptimizerException("OFCGST.notifyChange(): don't know "+
    ...
    // params.put("thread.id", new Integer(-1));  // indicate no thread id 
		                                              // knowledge
  ...
  private PairObjDouble min(FunctionIntf f, HashMap p) 
    ...
    if (grad==null) 
			grad = new GradApproximator(f);  // default: gradient numeric computation
    final VectorIntf x0 = 
    ...
    if (x0==null) 
			throw new OptimizerException("no fcg.x0"+
  ...
  private PairObjDouble min(FunctionIntf f, HashMap p) 
    ...
    for (int iter=0; iter<maxiters; iter++) {
      ...
      if (Double.compare(norminfg, gtol) <= 0) {
        mger.msg("FCGST.min(): found sol w/ value="+fx+" normg="+norminfg+
      ...
      if (Double.compare(sTg,0) >= 0) {  // reset search direction
        mger.msg("FCGST.min(): resetting search direction",0);
        for (int i=0; i<n; i++) // s.setCoord(i, -g.getCoord(i)/normg);
					s.setCoord(i, -g.getCoord(i));
        // sTg = -normg;
				sTg = -normg*normg;
      ...
      if (fbarD!=null) fbar = fbarD.doubleValue();
      h = FCGThread.findStepSize(f, grad, x, fx, s, sTg, fbar, 
				                         rho, sigma, t1, t2, t3, p);
      if (Double.compare(h, 0)<= 0) {
        mger.msg("FCGST.min(): FCG will stop at x=" + x +
                 " as it could not find a valid h", 0);
        return new PairObjDouble(x, fx);
      }
      // set new x
    ...
    if (found) return new PairObjDouble(x, fx);
    else throw new OptimizerException("FCGST did not find a solution "+
                                      "satisfying tolerance criteria from "+
                                      "the given initial point x0="+x0);
  }
